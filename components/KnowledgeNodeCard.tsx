...
const LEVEL_STYLES: Record<number, { card: string; badge: string; label: string }> = {
  0: {
    card: 'bg-purple-50 border-purple-300 hover:border-purple-500',
    badge: 'bg-purple-100 text-purple-700',
    label: 'Level 0 · 前置',
  },
  1: {
    card: 'bg-blue-50 border-blue-300 hover:border-blue-500',
    badge: 'bg-blue-100 text-blue-700',
    label: 'Level 1 · 核心',
  },
  2: {
    card: 'bg-green-50 border-green-300 hover:border-green-500',
...
export default function KnowledgeNodeCard({ data }: NodeProps) {
  ...
  return (
    <div
      ...
      className={`w-44 px-3 py-3 rounded-xl border-2 cursor-pointer shadow-sm hover:shadow-md transition-all select-none ${style.card}`}
    >
      {node.parent && (
        <Handle
          type="target"
